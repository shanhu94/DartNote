...
void declarationVariable() {
  ...
  print('num1\'s type ${num1 is int ? '==' : '!='} int ');
  print('num2: $num2');
}
...
void allowAnyType() {
  ...
  Object obj = 10;
  print('obj = $obj');
  // obj.toDouble(); 编译错误, Object可以承接所有类型, 但在调用对应类型的方法时, 需要先进行类型判断
  ...
  dynamic dObj = 20;
  print('dObj = $dObj');
  dObj.toDouble(); // dynamic 在编译时,也可以承接所有类型, 同时可以调用所有方法. 但在运行时如果对象没有对应的方法将会抛出异常.

  dObj = 'dObj string';
  print('dObj = $dObj');
  // dObj.toDouble(); // 此时将会在运行时抛出异常
